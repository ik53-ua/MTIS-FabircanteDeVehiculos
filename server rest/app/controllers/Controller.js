...
class Controller {
  ...
  */
  ...
  static collectRequestParams(request) {
    const requestParams = {};
    logger.info('📊 collectRequestParams - Extrayendo parámetros');
    ...
    if (request.openapi.schema.requestBody !== null) {
      ...
      if (content['application/json'] !== undefined) {
        ...
        logger.info(`  ✓ requestBodyName camelCase: "${requestBodyName}"`);
        logger.info(`  ✓ request.body completo:`, JSON.stringify(request.body));
        
        // Si request.body ya tiene los campos del request (componenteId, etc),
        // asignamos directamente sin envolverlo nuevamente
        if (request.body && Object.keys(request.body).length > 0) {
          requestParams[requestBodyName] = request.body;
          logger.info(`  ✓ Asignado requestParams[${requestBodyName}]:`, JSON.stringify(request.body));
        }
    ...
    logger.info('  📦 requestParams final:', JSON.stringify(requestParams));
